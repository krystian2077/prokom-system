...
export default function HammerFilms() {
  return (
    <section id="folie" className="bg-white lg:bg-[#fafafa] py-16 lg:py-24">
      <div className="mx-auto max-w-6xl px-5 lg:px-20">
        <div className="text-center">
          ...
          <h2 className="text-3xl lg:text-4xl font-bold leading-tight tracking-tight text-[#0d0d0d] sm:text-5xl lg:text-[3rem]">
            9 rodzajów folii —{" "}
            <span className="not-italic text-[#e11d1d]">
        ...
        <div className="mx-auto mt-10 lg:mt-16 grid max-w-[1140px] grid-cols-2 gap-3 lg:gap-4 lg:grid-cols-4">
          {films.map((film) => {
            ...
            return (
              <div
                key={film.name}
                className={
                  "relative overflow-hidden rounded-[20px] border-[1.5px] bg-white p-4 lg:p-6 shadow-[0_2px_8px_rgba(15,23,42,0.06),0_12px_28px_rgba(15,23,42,0.09)] lg:shadow-none transition-all duration-[250ms] hover:-translate-y-[6px] hover:border-[rgba(220,30,30,0.3)] hover:shadow-[0_12px_40px_rgba(0,0,0,0.1)] " +
                  (film.featured
                    ? "border-[rgba(225,29,29,0.5)] shadow-[0_4px_20px_rgba(225,29,29,0.12)]"
                    : "border-slate-100 lg:border-[rgba(0,0,0,0.07)]") +
                  (film.colSpan === 2 ? " col-span-2 lg:col-span-2" : "")
                }
              >
                <div
                ...
                />
                {film.featured && (
                  <span className="absolute right-3 top-3 lg:right-3.5 lg:top-3.5 rounded-full bg-[#e11d1d] px-2 lg:px-2.5 py-0.5 text-[8px] lg:text-[9px] font-extrabold uppercase tracking-[0.08em] text-white">
                    Premium
                  </span>
                )}

                <div
                  className={`${tier.textColor} mb-2 lg:mb-2.5 flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-[0.14em]`}
                >
                  <span className="h-1.5 w-1.5 rounded-full bg-current" />
                  {tier.label}
                </div>

                <h3 className="mb-1.5 lg:mb-2 font-syne text-[14px] lg:text-[15px] font-semibold leading-[1.2] tracking-[-0.01em] text-[#0d0d0d]">
                  {film.name}
                </h3>

                {/* Dots */}
                <div className="mb-3 lg:mb-4 flex gap-1.5">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <span
                      key={i}
                      className={
                        "h-[7px] w-[7px] rounded-full " +
                        (i < dots ? "bg-[#e11d1d]" : "bg-[#e5e7eb]")
                      }
                    />
                  ))}
                </div>
