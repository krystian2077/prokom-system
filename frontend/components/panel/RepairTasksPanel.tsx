  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-[#0c0d12] p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-[#9ca3af]">
        ...
        <Link
          ...
          className="shrink-0 rounded-2xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-[#93c5fd] transition hover:bg-white/10"
        >
          Moje zadania (filtr)
        </Link>
      </div>
        ...
        >
          <div className="flex gap-3">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold text-white">Masz już otwarte zadania przy tej naprawie</p>
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {suggestions.map((s) => (
              <div
                ...
                className="flex max-w-full flex-col gap-1 rounded-2xl border border-white/10 bg-[#0f1117] px-3 py-2 sm:max-w-[320px] sm:flex-row sm:items-center sm:gap-3"
              >
                ...
                <button
                  ...
                  className="shrink-0 rounded-xl bg-[#3b82f6] px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-[#2563eb] disabled:opacity-60"
                >
                  Dodaj
                </button>
              </div>
      ...
      <div className="mt-5 border-t border-white/10 pt-4">
        <div className="text-[11px] font-semibold uppercase tracking-wide text-[#6b7280]">Nowe zadanie</div>
        <p className="mt-1 text-xs text-[#6b7280]">
          Wpisz tytuł poniżej lub użyj szablonu — wstawi on tekst w pole (możesz go edytować).
        </p>
        <div className="mt-3 rounded-2xl border border-[#3b82f6]/25 bg-[#0f1117] p-3">
          ...
          <input
            ...
            autoComplete="off"
            className="w-full rounded-xl border border-white/10 bg-[#111318] px-3 py-2.5 text-sm text-white outline-none placeholder:text-[#6b7280] focus:border-[#3b82f6]"
          />
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <select
              value={composerPriority}
              onChange={(e) => setComposerPriority(e.target.value)}
              className="rounded-xl border border-white/10 bg-[#111318] px-3 py-2 text-sm text-white outline-none"
            >
            ...
            <button
              ...
              className="rounded-xl border border-white/10 px-4 py-2 text-sm font-semibold text-[#9ca3af] hover:bg-white/5"
            >
              Wyczyść pole
            </button>
          </div>
        ...
        <div className="mt-3 text-[11px] font-semibold uppercase tracking-wide text-[#6b7280]">Szablony (wypełniają pole)</div>
        <div className="mt-2 flex flex-wrap gap-2">
          {QUICK_TEMPLATES.map((t) => (
            <button
              ...
              className="rounded-xl border border-white/10 bg-[#0f1117] px-3 py-2 text-xs font-semibold text-[#e5e7eb] transition hover:border-[#3b82f6]/40 hover:bg-white/5"
            >
              {t.label}
            </button>
          ))}
      ...
      <div className="mt-5 border-t border-white/10 pt-4">
        ...
        {tasksQuery.isError ? (
          ...
          <p className="mt-2 text-sm text-[#6b7280]">Brak otwartych zadań przypisanych do Ciebie przy tej naprawie.</p>
        ) : (
          <ul className="mt-2 space-y-2">
            {openTasks.map((t) => (
              <li key={t.id}>
                <Link
                  href={zadaniaFilteredHref}
                  className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-white/10 bg-[#0f1117] px-3 py-2 text-sm transition hover:border-white/20 hover:bg-white/5"
                >
                  <span className="min-w-0 font-medium text-white">{t.title}</span>
                  <span className="shrink-0 text-[11px] font-semibold text-[#9ca3af]">{t.status_display ?? t.status}</span>
                </Link>
              </li>
            ))}
          </ul>
