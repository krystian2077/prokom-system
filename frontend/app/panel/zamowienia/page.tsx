  ...
  return (
    <main className="mx-auto min-h-screen max-w-6xl px-4 py-8">
      <header className="mb-6">
        <p className="text-xs uppercase tracking-[0.2em] text-[#9ca3af]">Panel Admina · Moduł</p>
        <h1 className="mt-2 text-2xl font-semibold text-white">Zamówienia i zaopatrzenie</h1>
        <p className="mt-1 text-sm text-[#9ca3af]">Dashboard oraz listy zamówień do realizacji.</p>
      </header>
            ...
            {(
              ...
              <button
                ...
                className={`rounded-xl border px-4 py-2 text-sm font-semibold transition ${
                  tab === k ? "border-white/20 bg-white/10 text-white" : "border-white/10 bg-white/5 text-[#9ca3af] hover:bg-white/10 hover:text-white"
                }`}
              >
                {label}
              </button>
              ...
              <>
                <div className="mb-4 flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-[#9ca3af]">Kafelki statusów oraz kluczowe metryki.</p>
                  </div>
                  <div className="text-right text-xs text-[#9ca3af]">
                    Profit: <span className="text-white font-semibold">{formatMoney(dashboard.total_profit)}</span>
                  </div>
                </div>
                ...
                <div className="mt-4">
                  <p className="text-sm text-[#9ca3af]">
                    Nieodebrane: <span className="text-white font-semibold">{dashboard.uncollected_count}</span>
                  </p>
                </div>
              </>
            )}
        ...
        {tab === "requires_action" ? (
          <div className="rounded-3xl border border-white/10 bg-[#0c0d12] p-4">
            {requiresLoading ? (
              ...
              <>
                <p className="text-sm font-semibold text-white">{tabLabel}</p>
                <p className="mt-1 text-sm text-[#9ca3af]">Lista do powiadomienia klienta oraz przeterminowane zamówienia.</p>

                <div className="mt-4 space-y-3">
                  {requiresAction.length === 0 ? (
                    <p className="text-sm text-[#6b7280]">Brak pozycji.</p>
                  ) : (
        ...
        {tab === "by_supplier" ? (
          <div className="rounded-3xl border border-white/10 bg-[#0c0d12] p-4">
            {bySupplierLoading ? (
              ...
              <>
                <p className="text-sm font-semibold text-white">{tabLabel}</p>
                <p className="mt-1 text-sm text-[#9ca3af]">Grupowanie zamówień po hurtowniach.</p>

                <div className="mt-4 space-y-4">
                  {bySupplier.by_supplier.length === 0 ? (
                    <p className="text-sm text-[#6b7280]">Brak hurtowni w danych.</p>
                  ) : (
                    bySupplier.by_supplier.map((s) => (
                      <div key={s.supplier.id} className="rounded-3xl border border-white/10 bg-[#0b0c10] p-4">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div>
                            <p className="text-sm font-semibold text-white">{s.supplier.name}</p>
                            <p className="mt-1 text-sm text-[#9ca3af]">
                              Klienci: <span className="text-white font-semibold">{s.customer_order_count}</span> · Sklep:{" "}
                              <span className="text-white font-semibold">{s.store_supply_count}</span>
                            </p>
                          </div>
                          <p className="text-sm text-[#9ca3af]">
                            Est. koszt: <span className="text-white font-semibold">{formatMoney(s.total_estimated_cost)}</span>
                          </p>
                        </div>

                        <div className="mt-4 grid gap-3 md:grid-cols-2">
                          <div className="space-y-3">
                            <p className="text-xs uppercase tracking-[0.12em] text-[#9ca3af]">Klienckie</p>
                          ...
                          <div className="space-y-3">
                            <p className="text-xs uppercase tracking-[0.12em] text-[#9ca3af]">Sklep</p>
...
function MetricCard({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-3xl border border-white/10 bg-white/5 p-4">
      <p className="text-xs uppercase tracking-[0.12em] text-[#9ca3af]">{label}</p>
...
function OrderCardCustomer({ order, compact }: { order: CustomerOrder; compact?: boolean }) {
  ...
  return (
    <div className={`rounded-2xl border border-white/10 bg-[#0b0c10] p-3 ${compact ? "p-3" : "p-4"}`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-[220px]">
          ...
          <p className="mt-1 text-sm text-[#9ca3af]">
            Ilość: <span className="text-white font-semibold">{order.quantity}</span>
          </p>
          {order.planned_order_date ? (
            <p className="mt-1 text-sm text-[#9ca3af]">
              Plan: <span className="text-white font-semibold">{formatDate(order.planned_order_date)}</span>
            </p>
          ) : null}
          {order.expected_delivery_date ? (
            <p className="mt-1 text-sm text-[#9ca3af]">
              Dotarcie: <span className="text-white font-semibold">{formatDate(order.expected_delivery_date)}</span>
            </p>
          ) : null}
        </div>
        ...
        <div className="text-right">
          ...
          {marginPercent !== null && marginPercent !== undefined ? (
            <p className="mt-1 text-xs text-[#9ca3af]">
              Marża%: <span className="text-white font-semibold">{marginPercent}%</span>
            </p>
          ) : null}
        </div>
      </div>
    ...
    <div className={`rounded-2xl border border-white/10 bg-[#0b0c10] p-3 ${compact ? "p-3" : "p-4"}`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-[220px]">
          ...
          <p className="mt-1 text-sm text-[#9ca3af]">
            Priorytet: <span className="text-white font-semibold">{order.priority_display}</span>
          </p>
          <p className="mt-1 text-sm text-[#9ca3af]">
            Koszt: <span className="text-white font-semibold">{formatMoney(order.estimated_cost ?? null)}</span>
          </p>
        </div>
        <div className="text-right">
          <span
...
function OrdersSectionCustomer({ orders }: { orders: CustomerOrder[] }) {
  return (
    <div className="rounded-3xl border border-white/10 bg-white/5 p-4">
      <p className="text-sm font-semibold text-white">Zamówienia klientów</p>
      <p className="mt-1 text-sm text-[#9ca3af]">Max 100 pozycji z endpointu „do dziś”.</p>
      <div className="mt-4 space-y-3">
        {orders.length === 0 ? <p className="text-sm text-[#6b7280]">Brak</p> : orders.slice(0, 30).map((o) => <OrderCardCustomer key={o.id} order={o} compact />)}
      </div>
    </div>
  );
}
...
function OrdersSectionSupply({ orders }: { orders: StoreSupplyOrder[] }) {
  return (
    <div className="rounded-3xl border border-white/10 bg-white/5 p-4">
      <p className="text-sm font-semibold text-white">Zaopatrzenie sklepu</p>
      <p className="mt-1 text-sm text-[#9ca3af]">Max 100 pozycji z endpointu „do dziś”.</p>
      <div className="mt-4 space-y-3">
        {orders.length === 0 ? <p className="text-sm text-[#6b7280]">Brak</p> : orders.slice(0, 30).map((o) => <OrderCardSupply key={o.id} order={o} compact />)}
      </div>
    </div>
  );
}
