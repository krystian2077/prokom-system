...
export default function WorkerNotificationDetailsPage() {
  ...
  const loadDetails = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await api.get<StaffNotificationItem>(`/accounts/notifications/${id}/`, token);
      setItem(data ?? null);
    } catch (e) {
      setError(e instanceof Error ? e : new Error("Nie udało się pobrać szczegółów powiadomienia."));
    } finally {
      setLoading(false);
    }
  }, [id, isAllowed, token]);
  ...
  const updateStatus = async (nextStatus: NotificationStatus) => {
    ...
    setSaving(true);
    try {
      const updated = await api.patch<StaffNotificationItem>(`/accounts/notifications/${item.id}/`, { status: nextStatus }, token);
      setItem(updated);
      ...
      setError(e instanceof Error ? e : new Error("Nie udało się zaktualizować statusu powiadomienia."));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <main className="mx-auto min-h-screen max-w-5xl px-4 py-8">
      <header className="mb-5 flex items-center justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-[0.2em] text-[#93a6cb]">Panel Pracownika</p>
          <h1 className="mt-2 text-2xl font-semibold text-white">Szczegóły powiadomienia</h1>
        </div>
        <Link href={p.powiadomieniaPath} className="rounded-xl border border-white/10 bg-white/[0.04] px-4 py-2 text-sm text-[#c4d2ec] transition hover:bg-white/[0.12] hover:text-white">
          Wróć do listy
        </Link>
      </header>
      ...
      {error && !loading ? (
        <div className="mb-4">
          <ErrorState error={error} onRetry={() => void loadDetails()} title="Nie udało się załadować powiadomienia" />
        </div>
      ) : null}
        ...
        <section className="rounded-3xl border border-[#2b3650] bg-gradient-to-b from-[#0d1424] to-[#0a0f1d] p-5 shadow-[0_16px_46px_rgba(0,0,0,.35)]">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            {item.repair_id ? (
              <button
                type="button"
                onClick={() => router.push(p.repairDetailPath(item.repair_id!))}
            ...
            {item.repair_id && isMessageNotificationType(item.notification_type) ? (
              <button
                type="button"
                onClick={() => router.push(`${p.commPath}?repairId=${encodeURIComponent(item.repair_id!)}`)}
                className="rounded-xl border border-[#dc1e1e]/40 bg-[#dc1e1e]/15 px-4 py-2 text-sm font-semibold text-[#fecaca] transition hover:bg-[#dc1e1e]/25"
              >
                Otwórz konwersację
              </button>
            ) : null}
