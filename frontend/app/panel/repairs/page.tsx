...
const REPAIRS_PILL_KEYS: RepairsPillKey[] = ["all", "new", "in_progress", "parts_waiting", "ready"];
...
function RepairsPageInner() {
  ...
  /** Nr | Urządzenie | Klient | Data przyjęcia | Status | Koszt | Telefon — wyśrodkowane, czytelna typografia */
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-[#d1d5db]">
              ...
              <span className="mt-1 block text-xs font-normal text-[var(--ink2)]">
                Kliknij wiersz, aby otworzyć szczegóły naprawy.
              </span>
            </p>
          </div>
          ...
          <div className="mt-4 overflow-x-auto rounded-2xl border border-[#2a3142]/80 bg-[#08090d] shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
            ...
            {repairsQuery.isLoading ? (
              ...
              <EmptyState
                ...
                description={
                  isAdmin ? "Po utworzeniu pierwszego zlecenia pojawi się ono tutaj." : EMPTY_STATES.myRepairs.description
                }
              />
            ) : filtered.length === 0 ? (
              <EmptyState
