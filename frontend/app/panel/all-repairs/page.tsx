...
function AllRepairsPageInner() {
  ...
  const repairsQuery = useQuery({
    queryKey: ["repairs", "all", pill, sort],
    enabled: Boolean(token),
    queryFn: async () => {
      if (!token) throw new Error("Missing auth/token");
      // Bez przypisania / staff-only: dla wszystkich napraw widocznych dla staff/admin
      ...
      <div className="flex flex-col gap-4">
        <header className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-semibold text-white">Wszystkie naprawy</h1>
            <p className="mt-1 text-sm text-[#9ca3af]">Podgląd: statusy, SLA i kolejka.</p>
          </div>
